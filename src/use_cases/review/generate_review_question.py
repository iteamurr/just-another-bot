...
class GenerateReviewQuestionUseCase:
    ...
    async def execute(
        self, command: GenerateReviewQuestionCommand
    ) -> GenerateReviewQuestionResult:
        ...
        if card.cached_question is not None:
            return GenerateReviewQuestionResult(
                card_id=card.id, question=card.cached_question
            )
        ...
        question = await self.llm_client.generate_review_question(
